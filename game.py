...
def relative_pos(pos):
    x = pos[0] / 690
    y = pos[1] / 400
    return Viewer.width * x, Viewer.height * y
...
class Moving_Platform():
    ...
    def __init__(self, speed, path ,x,y,  x1, y1, x2, y2, size, space,  friction = 2.0, group=1, color=(0,0,255)):
        # platforms container
        self.speed = speed
        #self.body = body
        self.path = path
        ...
        # put it in pymunk space
        #if self.kinematic:
        self.body = pymunk.Body(body_type=pymunk.Body.KINEMATIC)

        self.body.position = x, y
        self.s = pymunk.Segment(self.body, (x1, y1), (x2, y2), size)
        self.s.friction = friction
        self.s.group = group
        self.s.color = color    # = pygame.color.THECOLORS["blue"]
        space.add(self.s)
        # put everything into Viewer.platforms
        Viewer.moving_platforms.append(self)
...
class Viewer():
    ...
    def __init__(self, width=690, height=400, fps=60):
        ...
        # ----- walls and moving platforms (player must exist already!)
        self.change_level(self.current_level_number)

        # -------------- level_number: player-startpos
        self.leveldata = {1: (self.width * 0.1449275, self.height * 0.25),
                          2: (self.width * 0.1449275, self.height * 0.25)}

        self.run()

    def change_level(self, level_number):
        self.space.remove(self.body, self.head, self.feet, self.head2)
        self.space = levels.create_walls(level_number, self.space, self.width, self.height)
        self.space.gravity = (0, -1000)
        self.draw_options = pymunk.pygame_util.DrawOptions(self.screen)
        self.space.add(self.body, self.head, self.feet, self.head2)
        # moving platforms
        # kill all existing moving platforms and remove them from space
        for mp in Viewer.moving_platforms:
            self.space.remove(mp.s)
        Viewer.moving_platforms = []
        values = levels.create_moving_platforms(level_number, self.width, self.height)
        for v in values:
            Moving_Platform(speed=v[0], path=v[1], x=v[2], y=v[3],x1=v[4], y1=v[5], x2=v[6], y2=v[7], size=v[8], space=self.space)
    ...
    def run(self):
        ...
        while running:
            ...
            for mp in Viewer.moving_platforms:
                destination = mp.path[mp.path_index]
                current = pymunk.vec2d.Vec2d(mp.body.position)
                distance = current.get_distance(destination)
                if distance < mp.speed:
                    mp.path_index += 1
                    mp.path_index = mp.path_index % len(mp.path)
                    t = 1
                else:
                    t = mp.speed / distance
                new = current.interpolate_to(destination, t)
                mp.body.position = new
                mp.body.velocity = (new-current) / self.dt
            ...
            write(self.screen, "Move with Left/Right, jump with Up, press again to double jump", 100, 1,
                  (250, 250, 250), font_size=12)
